...
from app.account.schemas import PasswordChangeRequest, PasswordResetEmailRequest, PasswordResetRequest, UserCreate, UserOut, UserLogin
from fastapi.responses import JSONResponse
from app.db.config import SessionDep
from app.account.services import change_password, create_user, authenticate_user, email_verfication_send, password_reset_email_send, verify_email_token, verify_password_reset_token, email_verification_send_email
...
async def send_verification_email_email(
    background_tasks: BackgroundTasks, 
    user: User = Depends(get_current_user)
):
    return await email_verification_send_email(user, background_tasks)